  ...
  routes: [
    {
      ...
      handler: (async function (ctx, next) {
        ...
        try {
          const result = await transactions.users.createUserAndHouseholdByEmail(db, logger, {
            email,
            nickname,
            password,
          });
...

    {
      ...
      handler: (async function (ctx, next) {
        ...
        try {
          const result = await transactions.users.createUserAndHouseholdByDeviceIdentifier(db, logger, {
            deviceIdentifier,
            nickname,
          });
          ctx.status = 200;
